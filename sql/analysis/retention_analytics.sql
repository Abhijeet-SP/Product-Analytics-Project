...
SELECT 
    ...
    ROUND(100.0 * COUNT(*) FILTER (WHERE days_to_second <= 30) / NULLIF(COUNT(*), 0), 1) AS pct_second_within_30d,
    ROUND(100.0 * COUNT(*) FILTER (WHERE days_to_second <= 60) / NULLIF(COUNT(*), 0), 1) AS pct_second_within_60d,
    ROUND(100.0 * COUNT(*) FILTER (WHERE days_to_second <= 90) / NULLIF(COUNT(*), 0), 1)AS pct_second_within_90d,
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY days_to_second) AS median_days_to_second
FROM gaps;
